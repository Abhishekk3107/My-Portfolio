  ...
  return (
    <section id="projects" className="py-20 bg-transparent">
      <div className="container mx-auto px-6">
        <h2 className="text-3xl md:text-4xl font-bold text-center mb-8 text-white">My Projects</h2>
        <div className="flex flex-wrap justify-center gap-4 mb-8">
          <button onClick={() => setFilter('all')} className={`px-4 py-2 rounded-full ${filter==='all'?'bg-accent text-primary':'bg-[rgba(255,255,255,0.03)] text-muted'}`}>All</button>
          {techs.map(t => (
            <button key={t} onClick={() => setFilter(t)} className={`px-4 py-2 rounded-full ${filter===t?'bg-accent text-primary':'bg-[rgba(255,255,255,0.03)] text-muted'}`}>{t}</button>
          ))}
        </div>

        <div id="projects-grid" className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filtered.map((project, idx) => (
            <article key={project.title} className="project-card bg-[rgba(255,255,255,0.02)] rounded-lg overflow-hidden shadow-lg transition-all duration-300" style={{animationDelay: `${idx*0.1}s`}}>
              ...
              <div className="p-6">
                ...
                <div className="flex gap-3">
                  ...
                  <a href={project.liveUrl} target="_blank" rel="noreferrer" className="flex-1 bg-accent text-primary px-4 py-2 rounded text-center">Live</a>
                </div>
              </div>
            </article>
          ))}
