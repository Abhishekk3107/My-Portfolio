  ...
  return (
    <footer ref={footerRef} className="bg-[rgba(5,6,10,0.95)] backdrop-blur-sm border-t border-[rgba(255,255,255,0.08)] relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none opacity-5">
        <div className="absolute top-10 left-10 font-mono text-white/10 text-sm rotate-12">{'</>'}</div>
        <div className="absolute top-20 right-20 font-mono text-white/8 text-xs -rotate-6">{'dev'}</div>
        <div className="absolute bottom-20 left-1/4 font-mono text-white/12 text-lg rotate-45">{'{ }'}</div>
        <div className="absolute bottom-10 right-1/3 font-mono text-white/10 text-sm -rotate-12">{'code'}</div>
      </div>
      ...
      <div className="container mx-auto px-6 py-16 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12 mb-12">
          ...
          <div className="footer-section space-y-6">
            <div className="flex items-center gap-3">
              <div 
                ref={logoRef}
                className="w-12 h-12 flex items-center justify-center rounded-full bg-[linear-gradient(135deg,var(--accent),var(--accent-2))] text-white font-bold cursor-pointer"
              >
                <span className="text-lg font-extrabold">AK</span>
              </div>
              <div>
                <h3 className="text-xl font-bold text-white">Abhishek Kumar</h3>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3 text-muted text-sm">
                <EmailIcon className="w-4 h-4 text-accent flex-shrink-0" />
                <a
              ...
              <div className="flex items-center gap-3 text-muted text-sm">
                <LocationIcon className="w-4 h-4 text-accent flex-shrink-0" />
            ...
            <ul ref={linksRef} className="space-y-3">
              {quickLinks.map((link) => (
                <li key={link.name}>
                  <a
                    href={link.href}
                    className="text-muted hover:text-accent transition-colors duration-300 flex items-center gap-2 group text-sm"
                  >
          ...
          <div className="footer-section space-y-6">
            ...
            <div ref={socialRef} className="flex gap-3">
              {data?.personal?.social && Object.entries(data.personal.social).slice(0, 3).map(([platform, url]) => (
                <a
                  ...
                  className="w-10 h-10 bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] rounded-lg flex items-center justify-center text-muted hover:text-white hover:border-accent/50 hover:bg-accent/10 transition-all duration-300"
                  title={platform}
                >
            ...
            {/* Technologies */}
            <div>
              <h5 className="text-white font-medium mb-3 text-sm">Technologies</h5>
        ...
        {/* Divider */}
        <div className="footer-divider h-px bg-gradient-to-r from-transparent via-[rgba(255,255,255,0.1)] to-transparent mb-8"></div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <button
            onClick={scrollToTop}
            className="group flex items-center gap-2 px-4 py-2 bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] rounded-lg text-muted hover:text-white hover:border-accent/50 hover:bg-accent/10 transition-all duration-300"
            title="Back to top"
          >
